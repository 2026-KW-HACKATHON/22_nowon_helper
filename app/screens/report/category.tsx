...
import { classifyPhoto, checkNearby } from '@/report/api';
import { CLASSIFY_MIN_CONFIDENCE } from '@/report/config';
import { updateDraft, useDraft } from '@/report/draft';
import { CATEGORIES, CATEGORY_LABEL } from '@/report/labels';
...
export default function CategoryScreen() {
  ...
  async function next() {
    ...
    try {
      ...
      router.push(duplicate ? '/screens/report/duplicate' : '/screens/report/details');
    } catch {
      // The duplicate check must never block a report — go straight to the form.
      updateDraft({ duplicate: null });
      router.push('/screens/report/details');
    } finally {
      setBusy(false);
    }
  }
