...
import { ApiRequestError, confirmReport } from '@/report/api';
import { updateDraft, useDraft } from '@/report/draft';
import { CATEGORY_LABEL, GROUP_LABEL, STATUS_LABEL } from '@/report/labels';
  ...
  return (
    <SafeAreaView style={s.screen} edges={['bottom']}>
      <ScrollView contentContainerStyle={s.content}>
        <Text style={s.lead}>
          {report.distance_m != null ? `${report.distance_m}m 거리` : `반경 ${DUPLICATE_RADIUS_M}m 안`}에 같은 문제가 이미
          신고되어 있어요.
        </Text>
        ...
        <View style={s.card}>
          ...
          <View style={s.cardBody}>
            <View style={s.row}>
              ...
              <Text style={s.status}>{STATUS_LABEL[report.status]}</Text>
            </View>
      ...
      <View style={s.footer}>
        {result || alreadyConfirmed ? (
          ...
          <>
            ...
            <SecondaryButton label="다른 문제예요 · 새로 신고하기" onPress={() => router.push('/screens/report/details')} />
          </>
        )}
      </View>
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  status: { color: C.amber, fontSize: 16, fontWeight: '700' },
